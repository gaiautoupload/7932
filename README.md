...
先在 `../7932_strategy/config.json` 設定每日檔來源，執行 `python tracker.py`。追蹤器會從歷史資料重新評估 20 個疑似核心主力分點，計算當日、近 5、近 10、近 20 個交易日的買賣超張數與金額，並覆寫 `data/snapshot.json`；提交並推送該檔後，GitHub Pages 會自動顯示最新資料。
